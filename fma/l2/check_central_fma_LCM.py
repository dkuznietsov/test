...
PK_COLS_LIST = ['FAB_CODE', 'PROCESS_STAGE', 'TFT_CHIP_ID_RRN', 'FMA_TIME']

# and T.TEST_TIME = S.TEST_TIME
JOIN_STR = """
  T.FAB_CODE = S.FAB_CODE
  and T.PROCESS_STAGE = S.PROCESS_STAGE 
  and T.FMA_TIME = S.FMA_TIME 
...
#時間KEY值 變更時要改掉
def filter_time(df):
  return df.filter(f"FMA_TIME >= '{START_TIME}' AND FMA_TIME < '{END_TIME}'")
...
def transform_cols(df):
  return (
    ...
    .withColumn("TEST_TIME", F.date_format(F.col(f"TEST_TIME"), "yyyy-MM-dd HH:mm:s:ss"))
    .withColumn("FMA_TIME", F.date_format(F.col(f"FMA_TIME"), "yyyy-MM-dd HH:mm:s:ss"))   
  )
...
def get_sorce_dataframe():
  return spark.table("auofabdev.prod_dw.mf_all_s_all_dw_at_feedback_h_dax_fbk_fma_ods").transform(filter_time).transform(filter_process_stage_and_fab, CHECK_TYPE).transform(transform_cols).transform(select_stage_cols, is_source=True)
  
def get_target_dataframe():
  return spark.table("auofab.prod_dw.dt_all_s_all_dw_at_feedback_h_dax_fbk_fma_ods").transform(filter_time).transform(filter_process_stage_and_fab, CHECK_TYPE).transform(transform_cols).transform(select_stage_cols, is_source=False)
