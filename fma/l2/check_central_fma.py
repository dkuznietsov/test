...
START_UPDATE_DTM = '2025-02-14 00:00:00'  
END_UPDATE_DTM = '2025-02-15 00:00:00'
...
def check_cols(df):
  #比對欄位
  stage_cols_lists = ['SIGNAL_NO', 'GATE_NO','POX_X', 'POX_Y', 'MFG_DAY']
...
WHERE 1=1 
      ...
      AND FMA_TIME >= '{START_UPDATE_DTM}'  AND FMA_TIME < '{END_UPDATE_DTM}' 
      AND TRIM(PROCESS_STAGE) IN ({formatted_filter_ps_values}) 
"""
...
#merge key NULL補-
df_source = spark.sql(sql_source_str).withColumn("JUDGE_CNT", F.when(
                F.col("JUDGE_CNT").isNull(),
                F.lit('-')).otherwise(F.col("JUDGE_CNT")))
df_fusion = spark.sql(sql_fusion_str)
# df_source.cache()
# df_fusion.cache()
