    ...
    def revised_job(self) -> None:
        ...
        def fab2(stage: str):
            fab_dict = {
                ...
                'EDI': ['Z19', 'Z83', 'Z1F', 'Z1D', 'G23', 'G25', 'Z1L', 'Z1S', 'Z1T', 'G30'],
                'BRISA': ['Z40', 'Z45', 'Z31', 'Z0N'],
                'LCM_BEOL': ['S01', 'S02', 'S11'],
            }
            print(stage)
            return fab_dict[stage]

        for stage in ['MODULE', 'EDI', 'BRISA', 'LCM_BEOL']:
            fab_list = fab2(stage)
            ...
            for fab in fab_list:
                ...
                fab_set_l2_arg = {
                    ...
                    "run_if": "ALL_SUCCESS",
                    "notebook_task": {
                    "notebook_path": "scrap/l2/l2_set_job_args",
                    "base_parameters": {
                        "START_TIME": "",
                        "END_TIME": "",
                        "FAB": fab,
                        "JOB_NAME": f"{fab}_{stage}_F2C_H_DAX_FBK_SCRAP",
                        "PROCESS_STAGE": stage
                    },
                    "source": "GIT"
                    },
                ...
                master = {
                        "task_key": f"{fab}_{stage.lower()}_h_dax_fbk_scrap",
                        "depends_on": [
                        {
                            "task_key": f"{fab}_{stage.lower()}_set_l2_args"
                        }
                        ...
                        "run_if": "ALL_SUCCESS",
                        "notebook_task": {
                        "notebook_path": f"scrap/l2/main_{stage.lower()}",
                        "base_parameters": {
                            "TASK_KEY":f"{fab}_{stage.lower()}_set_l2_args"
                        },
                        "source": "GIT"
...
# env = "DEV"
# job_id = 600515801291801
env = "PRD"
job_id = 534426267813974
job_name = "FBK_Scrap_Module_ODSToCentral"
branch = "main"
L2_L3 = JobMigration(deploy_env=env, which_job="SCRAP")
