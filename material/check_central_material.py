...
def check_cols(df):
  #比對欄位
  stage_cols_lists = ['MATERIAL_PART_NO']
...
# DBTITLE 1,比對資料
table_tag = f"h_dax_fbk_material_ods"
process_stage = "CELL" # ARRAY CELL
START_UPDATE_DTM = '2024-12-16 12:00:00'  
END_UPDATE_DTM = '2024-12-16 23:59:00'
PK_COLS_LIST = ['PROCESS_STAGE','FAB_CODE','TFT_CHIP_ID_RRN','MATERIAL_TYPE','MATERIAL_LOT_ID']#key值
...
WHERE 1=1 
      --AND FAB_CODE = '101860' 
      AND FAB_CODE in ({formatted_filter_fabcode_values})
      AND UPDATE_DTM >= '{START_UPDATE_DTM}'  AND UPDATE_DTM < '{END_UPDATE_DTM}' 
      AND TRIM(PROCESS_STAGE) IN ({formatted_filter_ps_values}) 
"""
print(f"Source SQL:{sql_source_str}")
print(f"Fusion SQL:{sql_fusion_str}")
...
#merge key NULL補-
df_source = spark.sql(sql_source_str).withColumn("MATERIAL_TYPE", F.when(
                F.col("MATERIAL_TYPE").isNull(),
                F.lit('-')).otherwise(F.col("MATERIAL_TYPE")))
df_fusion = spark.sql(sql_fusion_str)
# df_source.cache()
# df_fusion.cache()
...
df = df_source.alias('S').join(df_fusion.alias('T'), on=PK_COLS_LIST , how='right')#改rigjt join 已後面那張為主
