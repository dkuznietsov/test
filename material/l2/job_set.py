...
workflow_path_dict = {
    "MODULE": "../app_config/material.json",
}
...
class JobMigration:
    ...
    def revised_job(self) -> None:
        ...
        def fab2(stage: str):
            fab_dict = {
                'MODULE': ['K01', 'K06', 'L6K', 'L8B', 'M02', 'M11', 'S01', 'S02', 'S06', 'S11'],
                'BRISA': ['X13', 'Z0M', 'Z45', 'S13', 'Z1E', 'Z31', 'Z40', 'ZK7', 'Z0W', 'Z0H', 'Z1F', 'Z0D', 'Z1L', 'ZF2', 'ZKC', 'Z22', 'Z94'],
                'LCMBEOL': ['S01', 'S02'],
                'EDI': ['Z19', 'Z1F', 'G23', 'G25', 'Z1L', 'Z1S', 'Z1T', 'G30', 'Z83', 'Z1D', 'ZB1'],
            }
            print(stage)
            return fab_dict[stage]

        for stage in ['BRISA', 'EDI']:
            fab_list = fab2(stage)
            ...
            for fab in fab_list:
                ...
                fab_set_l2_arg = {
                    ...
                    "run_if": "ALL_SUCCESS",
                    "notebook_task": {
                    "notebook_path": "material/l2/l2_set_job_args",
                    "base_parameters": {
                        "START_TIME": "",
                        "END_TIME": "",
                        "FAB": fab,
                        "JOB_NAME": f"{fab}_{stage}_F2C_H_DAX_FBK_MATERIAL",
                        "PROCESS_STAGE": stage
                    },
                    "source": "GIT"
                    },
                ...
                master = {
                        "task_key": f"{fab}_{stage.lower()}_h_dax_fbk_material",
                        "depends_on": [
                        {
                            "task_key": f"{fab}_{stage.lower()}_set_l2_args"
                        }
                        ...
                        "run_if": "ALL_SUCCESS",
                        "notebook_task": {
                        "notebook_path": f"material/l2/main_{stage.lower()}",
                        "base_parameters": {
                            "TASK_KEY":f"{fab}_{stage.lower()}_set_l2_args",
                        },
                        "source": "GIT"
...
env = "PRD"
job_id = 447112617778014     
job_name = "FBK_Material_EDI_Brisa_Module_ODSToCentral"
branch = "main"
