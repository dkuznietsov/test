...
from test.l2.src.running.array import ArrayTest
from test.l2.logic.stage_args import ArrayLogicArgs
...
if IS_TEST:
    ...
    if not JOB_NAME.strip():  # 如果去掉空格后字符串为空
        JOB_NAME = f"{FAB}_{PROCESS_STAGE}_F2C_H_DAX_FBK_TEST"  
...
if (JOB_RUN and IS_RUN == "TRUE"):
    ...
    try:
        ...
        Steps = {
            "test": KpiInfo("None", ['L3C','L3D','L4A','L5B','L5C','L5D','L6A','L6B','L6K','L7A','L7B','L8A','L8B'], logic_args)
        }
