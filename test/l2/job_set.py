...
workflow_path_dict = {
    "TEST_MODULE": "../app_config/test_module.json",
}
...
class JobMigration:
    ...
    def revised_job(self) -> None:
        ...
        def fab2(stage: str):
            fab_dict = {
                'MODULE': ['M02', 'M11', 'S01', 'K01', 'S02', 'S03', 'S11', 'S06', 'K06', 'L6K', 'S17', 'L6B', 'L8B'],
                'BRISA': ['S13','X13','Z0D','Z0H','Z0M','Z0W','Z1E','Z1F','Z1L','Z22','Z31','Z40','Z45','Z94','ZF2','ZK7','ZKC'],
                'LCM_BEOL': ['S01', 'S02'],
                'EDI': ['F20','G23','G25','G30','V05','Z0S','Z19','Z1D','Z1F','Z1L','Z1S','Z1T','Z83','ZB1'],
                'EDI_BEOL': ['F20','Z0S']
            }
            print(stage)
            return fab_dict[stage]

        for stage in ['MODULE', 'BRISA', 'LCM_BEOL', 'EDI', 'EDI_BEOL']:
            fab_list = fab2(stage)
            ...
            for fab in fab_list:
                ...
                #     "run_if": "ALL_SUCCESS",
                #     "notebook_task": {
                #     "notebook_path": "test/l2/l2_set_job_args",
                #     "base_parameters": {
                #         "START_TIME": "",
                #         "END_TIME": "",
                #         "FAB": fab,
                #         "JOB_NAME": f"{fab}_{stage}_F2C_H_DAX_FBK_TEST",
                #         "PROCESS_STAGE": stage
                #     },
                #     "source": "GIT"
                #     },
                ...
                master = {
                        "task_key": f"{fab}_{stage.lower()}_h_dax_fbk_test",
                        "depends_on": [
                        {
                            "task_key": f"job_status_run"
                        }
                        ],
                        "run_if": "ALL_SUCCESS",
                        "notebook_task": {
                        "notebook_path": f"test/l2/main_{stage.lower()}",
                        "base_parameters": {
                            "TASK_KEY":f"set_l2_args",
                            "FAB": fab,
                            "PROCESS_STAGE": stage
                        },
                        "source": "GIT"
        ...
        fab_set_l2_arg = {
                ...
                "notebook_path": "test/l2/l2_set_job_args",
                "base_parameters": {
                    "JOB_NAME": f"LCM_F2C_H_DAX_FBK_TEST",
                },
                "source": "GIT"
                },
                "job_cluster_key": "Job_cluster",
        ...
        job_run = {
                ...
                "run_if": "ALL_SUCCESS",
                "notebook_task": {
                "notebook_path": "test/l2/job_status",
                "base_parameters": {
                    "TASK_KEY": f"set_l2_args",
                    "JOB_STATUS": f"RUN",
                },
        ...
        table_vacuum_optimize = {
                "task_key": f"test_table_vacuum_optimize",
                "run_if": "ALL_SUCCESS",
                "notebook_task": {
                "notebook_path": "test/l2/table_vacuum_optimize",
                "base_parameters": {
                    "TABLE": f"TEST"
                },
                "source": "GIT"
        ...
        dest_job['settings']['tasks'].append(fab_set_l2_arg) 
        dest_job['settings']['tasks'].append(table_vacuum_optimize) 
        dest_job['settings']['tasks'].append(job_run) 
...
# env = "DEV"
# job_id = 225484336460450
# job_name = "FBK_Test_Module_ODSToCentral"
# branch = "main"
# L2_L3 = JobMigration(deploy_env=env, which_job="TEST_MODULE")
# L2_L3.reset_job(stage="MODULE", target_job_id=job_id, target_job_name=job_name, target_branch=branch)

# COMMAND ----------

env = "DEV"
job_id = 216999765698135
job_name = "FBK_Test_Module_ODSToCentral"
branch = "main"
L2_L3 = JobMigration(deploy_env=env, which_job="TEST_MODULE")
L2_L3.reset_job(stage="MODULE", target_job_id=job_id, target_job_name=job_name, target_branch=branch)
