...
workflow_path_dict = {
    "MODULE": "../app_config/defect_module.json",
}
...
class JobMigration:
    ...
    def revised_job(self) -> None:
        ...
        def fab2(stage: str):
            fab_dict = {
                'MODULE': ['M02','M11','S01', 'K01', 'S02', 'S11', 'S06', 'K06', 'L6K', 'L8B'],
                'BRISAMODULE': ['S13','X13','Z0D','Z0M','Z1E','Z1F','Z1L','Z22','Z31','Z45','Z94','ZF2','ZK7','ZKC','Z40'],
                'LCMBEOL': ['S01', 'S02'],
                'EDIMODULE': ['V05','ZB1','Z83','Z1T','Z1S','Z1L','Z1F','Z1D','Z19','Z0S','G30','G25','G23','F20'],
            }
            print(stage)
            return fab_dict[stage]

        for stage in ['MODULE', 'EDIMODULE', 'BRISAMODULE', 'LCMBEOL']:
            fab_list = fab2(stage)
            ...
            for fab in fab_list:
                ...
                fab_set_l2_arg = {
                    ...
                    "run_if": "ALL_SUCCESS",
                    "notebook_task": {
                    "notebook_path": "defect/l2/l2_set_job_args",
                    "base_parameters": {
                        "START_TIME": "",
                        "END_TIME": "",
                        "FAB": fab,
                        "JOB_NAME": f"{fab}_{stage}_F2C_H_DAX_FBK_DEFECT",
                        "PROCESS_STAGE": stage
                    },
                    "source": "GIT"
                    },
                ...
                master = {
                        "task_key": f"{fab}_{stage.lower()}_h_dax_fbk_defect",
                        "depends_on": [
                        {
                            "task_key": f"{fab}_{stage.lower()}_set_l2_args"
                        }
                        ...
                        "run_if": "ALL_SUCCESS",
                        "notebook_task": {
                        "notebook_path": f"defect/l2/main_{stage.lower()}",
                        "base_parameters": {
                            "TASK_KEY":f"{fab}_{stage.lower()}_set_l2_args",
                        },
                        "source": "GIT"
...
env = "DEV"
job_id = 874794283048279     
job_name = "FBK_Defect_Module_ODSToCentral"
branch = "Defect"
L2_L3 = JobMigration(deploy_env=env, which_job="MODULE")
