...
# START_UPDATE_DTM = '2024-12-14 06:15:00'  
...
# def check_cols(df):
#   #比對欄位
#   stage_cols_lists = ['FIRST_YIELD_FLAG', 'FINAL_YIELD_FLAG']
...
# DBTITLE 1,比對資料
# table_tag = f"h_dax_fbk_defect_ods"
# process_stage = "CELL" # ARRAY CELL
# START_UPDATE_DTM = '2024-12-18 11:15:00'  
# END_UPDATE_DTM = '2024-12-18 23:45:00'
# PK_COLS_LIST = ['PROCESS_STAGE','FAB_CODE','TFT_CHIP_ID_RRN','TEST_TYPE','TEST_TIME','DEFECT_CODE','TEST_SIGNAL_NO','TEST_GATE_NO']
...
# #merge key NULL補-
# df_source = spark.sql(sql_source_str).withColumn("DEFECT_CODE", F.when(
#                 F.col("DEFECT_CODE").isNull(),
#                 F.lit('-')).otherwise(F.col("DEFECT_CODE")))
# df_source = spark.sql(sql_source_str).withColumn("TEST_SIGNAL_NO", F.when(
#                 F.col("TEST_SIGNAL_NO").isNull(),
#                 F.lit('-')).otherwise(F.col("TEST_SIGNAL_NO")))
# df_source = spark.sql(sql_source_str).withColumn("TEST_GATE_NO", F.when(
#                 F.col("TEST_GATE_NO").isNull(),
#                 F.lit('-')).otherwise(F.col("TEST_GATE_NO")))                
# df_fusion = spark.sql(sql_fusion_str)
# # df_source.cache()
# # df_fusion.cache()
...
# %sql
# select * from auofab.itgp_eng.l8B_array_h_dax_fbk_test_ods T 
# WHERE TRIM(T.PROCESS_STAGE) IN ('ARRAY')
# AND T.UPDATE_DTM >= '2024-12-10 09:00:00'  
# AND T.UPDATE_DTM < '2024-12-10 10:00:00' 
# AND TFT_CHIP_ID_RRN IN ('104660396432951')
# --AND ARRAY_LOT_ID IN ('ZH4KC6W200')
# AND TEST_TYPE='LASER'

# COMMAND ----------

# %sql
# select * from auofabDEV.prod_dw.MF_all_s_all_dw_at_feedback_h_dax_fbk_test_ods T 

# WHERE TRIM(T.PROCESS_STAGE) IN ('ARRAY')
# AND T.FAB_CODE = '101860' 
# --AND ARRAY_LOT_ID IN ('ZH4KC6W200')
# AND TFT_CHIP_ID_RRN IN ('104660396432951')
# AND TEST_TYPE='LASER'
...
# %sql
# select * from auofab.prod_dw.DT_all_s_all_dw_at_feedback_h_dax_fbk_test_ods T 

# WHERE TRIM(T.PROCESS_STAGE) IN ('ARRAY')
# --AND T.UPDATE_DTM >= '2024-12-10 09:00:00'  
# --AND T.UPDATE_DTM < '2024-12-10 10:00:00' 
# AND TFT_CHIP_ID_RRN IN ('104660396432951')
# AND TEST_TIME= '2023-12-16 18:24:32'
# --AND ARRAY_LOT_ID IN ('ZH4KC6W200')
# AND TEST_TYPE='LASER'
...
PK_COLS_LIST = ["FAB_CODE", "PROCESS_STAGE", "TFT_CHIP_ID_RRN", "TEST_TYPE", "DEFECT_CODE", "TEST_SIGNAL_NO", "TEST_GATE_NO", "TEST_TIME", "DEFECT_VALUE", "TEST_POSITION"]

JOIN_STR = """
  T.FAB_CODE = S.FAB_CODE
  and T.PROCESS_STAGE = S.PROCESS_STAGE
  and T.TEST_TYPE = S.TEST_TYPE
  and T.DEFECT_CODE = S.DEFECT_CODE
  and T.TEST_SIGNAL_NO = S.TEST_SIGNAL_NO
  and T.TEST_GATE_NO = S.TEST_GATE_NO
  and T.TEST_TIME = S.TEST_TIME
  and CAST(T.TFT_CHIP_ID_RRN AS STRING) = CAST(S.TFT_CHIP_ID_RRN AS STRING)
  and T.DEFECT_VALUE <=> S.DEFECT_VALUE
  and T.TEST_POSITION <=> S.TEST_POSITION
"""
  # and T.TEST_TOOL_ID <=> T.TEST_TOOL_ID
...
def get_check_cols():
  _dict = [
        ...
        # "FINAL_YIELD_FLAG",
        "CHIP_TYPE",
        "ORIGINAL_DEFECT_CODE",
        "ORIGINAL_DEFECT_CODE_DESC",
        "ORIGINAL_TEST_TIME",
        "ORIGINAL_TEST_USER",
        "FMA_TIME",
        "FMA_MFG_DAY",
...
def transform_cols(df):
  return (
    df
    .withColumn("FAB_CODE", F.col("FAB_CODE").cast(DecimalType(6, 0)))
    .withColumn("TEST_TYPE", F.when(F.col("TEST_TYPE").isNull(), "").otherwise(F.regexp_replace("TEST_TYPE", "-", "")))
    .withColumn("TEST_TIME", F.date_format(F.col(f"TEST_TIME"), "yyyy-MM-dd HH:mm:ss"))
    .withColumn("TEST_SIGNAL_NO", F.col("TEST_SIGNAL_NO").cast(DecimalType(10, 0)))
    .withColumn("TEST_GATE_NO", F.col("TEST_GATE_NO").cast(DecimalType(10, 0)))
    .withColumn("DEFECT_VALUE", F.col("DEFECT_VALUE").cast(DecimalType(10, 0)))
  )

def filter_time(df):
  return df.filter(f"TEST_TIME >= '{START_TIME}' AND TEST_TIME < '{END_TIME}' AND DEFECT_CODE IS NOT NULL")

def filter_az_time(df):
  return df.filter(f"AZ_DB_TIME <= '{END_TIME}'")
...
def filter_process_stage_and_fab(df, check_type):
  if check_type == 'ARRAY':
    ...
    return df.filter(f"PROCESS_STAGE in ('BEOL') AND FAB_CODE NOT IN (115270,105000,115000,115060)")
  elif check_type == 'MODULE':
    return df.filter(f"PROCESS_STAGE in ('MODULE') AND FAB_CODE IN (105060,105160,111690,115000,115060,115160,115200,117006,117011,101860)")
    ...
    return df.filter(f"FAB_CODE IN (115400, 117381, 117912, 107103, 102455, 117133, 117021, 117330, 117410, 117480, 117520, 117940, 117918, 117920, 117919)")
  else:
    return df.filter(f""" PROCESS_STAGE NOT IN ('ARRAY', 'FEOL', 'BEOL') AND FAB_CODE NOT IN (117118, 117027, 117511, 117830, 116500, 117114, 117385, 117133, 117021, 189001, 189002, 117540, 117475, 127100, 115400, 117381, 117912, 107103, 102455, 117133, 117021, 117330, 117410, 117480, 117520, 117940, 117918, 117920, 117919) AND (PROCESS_STAGE IN ('MODULE') AND FAB_CODE NOT IN (105060,105160,111690,115000,115060,115160,115200,117006,117011,101860)) AND FAB_CODE NOT IN (115270,105000)""")
...
def get_sorce_dataframe(check_type):
  return spark.table("auofab.prod_dw.mf_all_s_all_dw_at_feedback_h_dax_fbk_defect_ods").transform(filter_time).transform(filter_process_stage_and_fab, check_type).transform(select_stage_cols, is_source=True).transform(transform_cols)
  
def get_target_dataframe(check_type):
  return spark.table("auofab.prod_dw.dt_all_s_all_dw_at_feedback_h_dax_fbk_defect_ods").transform(filter_time).transform(filter_az_time).transform(filter_process_stage_and_fab, check_type).transform(select_stage_cols, is_source=False).transform(transform_cols)
...
def check_data(check_type: str):
  ...
  check_all_col_df = (
    check_df
    .groupby("T.FAB_CODE", "T.SITE_ID", "T.PROCESS_STAGE", "NG_COLS")
    .count()
    .withColumn("FAB_CODE", F.col("FAB_CODE").cast("string"))
    )
  # check_all_col_df.display()
  check_all_col_df = check_all_col_df
...
if IS_SEND_MAIL:
  send_mail(
    ...
    """,
    send_from = f"FBK Defect 資料比對結果",
    send_to = ['Cory.CC.Cheng@auo.com', 'Perry.Chien@auo.com', 'steven.Yeh@auo.com'],
    send_subject = f"FBK Defect 驗證不一致: START_TIME = {START_TIME}, END_TIME = {END_TIME}"
  )
else:
  print(f"資料比對一致")
