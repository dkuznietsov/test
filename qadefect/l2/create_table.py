...
target_delta_table_location = f"{save_path}/mf_all_s_all_dw_at_feedback_h_dax_fbk_qadefect_ods"
tablename = f"{db_schema}.mf_all_s_all_dw_at_feedback_h_dax_fbk_qadefect_ods"

from pyspark.sql.functions import col

df = (
  spark.table(f"auofab.prod_dw.dt_all_s_all_dw_at_feedback_h_dax_fbk_qadefect_ods")
...
spark.sql(f'ALTER TABLE {db_schema}.mf_all_s_all_dw_at_feedback_h_dax_fbk_qadefect_ods SET TBLPROPERTIES (delta.autoOptimize.optimizeWrite = true, delta.autoOptimize.autoCompact = true)')
spark.sql(f'ALTER TABLE {db_schema}.mf_all_s_all_dw_at_feedback_h_dax_fbk_qadefect_ods CLUSTER BY (PROCESS_STAGE, FAB_CODE)') 
