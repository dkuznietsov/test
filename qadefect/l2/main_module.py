...
from qadefect.config import Config   # Config 放在各自的資料夾
from qadefect.l2.config.kpi_info import TableInfo, KpiInfo
from qadefect.l2.src.running.module import ModuleQaDefect
from qadefect.l2.logic.stage_args import ModuleLogicArgs
...
# Table Name
QADEFECT_TABLE = config.h_dax_fbk_qadefect_ods
QADEFECT_TABLENAME = config.mf_all_s_all_dw_at_feedback_h_dax_fbk_qadefect_ods
print(f"QADEFECT_TABLE:{QADEFECT_TABLE} , QADEFECT_TABLENAME:{QADEFECT_TABLENAME}")
...
if IS_TEST:
    ...
    if not JOB_NAME.strip():  # 如果去掉空格后字符串为空
        JOB_NAME = f"{FAB}_{PROCESS_STAGE}_F2C_H_DAX_FBK_QADEFECT"  
...
if (JOB_RUN and IS_RUN == "TRUE"):
    ...
    try:
        ...
        Steps = {
            "qadefect": KpiInfo("None", ['S01','S02','S06','L6K','S11'], logic_args)            
        }

        Info = TableInfo(
            fab = FAB,
            process_stage=PROCESS_STAGE,
            target_table=QADEFECT_TABLE,
            target_table_name=QADEFECT_TABLENAME,
            db_schema=db_schema,
            save_path=save_path,
        ...
        qadefect = ModuleQaDefect(
            table_info=Info,
        )
        qadefect.prepare_data()
        qadefect.delete_upsert_data()
