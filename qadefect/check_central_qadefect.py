...
START_UPDATE_DTM = '2024-12-19 13:00:00'  
END_UPDATE_DTM = '2024-12-14 15:15:00'

# COMMAND ----------

# DBTITLE 1,比對 sql
table_tag = f"h_dax_fbk_qa_ods"
...
# DBTITLE 1,驗證資料
PK_COLS_LIST = ['PROCESS_STAGE','FAB_CODE','TFT_CHIP_ID_RRN','TEST_TIME']
...
START_UPDATE_DTM = '2024-12-16 03:40:00'  
END_UPDATE_DTM = '2024-12-16 14:00:00'
PK_COLS_LIST =  ['PROCESS_STAGE','FAB_CODE','TFT_CHIP_ID_RRN','TEST_TIME','JUDGE_CRIT']
...
sql_source_str = f"""
SELECT TFT_CHIP_ID_RRN, PROCESS_STAGE, FAB_CODE, TEST_TIME, JUDGE_CRIT, COUNT(*) as count
...
WHERE 1=1 
      ...
      AND TRIM(PROCESS_STAGE) IN ({formatted_filter_ps_values})
GROUP BY TFT_CHIP_ID_RRN, PROCESS_STAGE, FAB_CODE, TEST_TIME, JUDGE_CRIT
"""
...
#merge key NULL補-
df_source = spark.sql(sql_source_str).withColumn("JUDGE_CRIT", F.when(
                F.col("JUDGE_CRIT").isNull(),
                F.lit('-')).otherwise(F.col("JUDGE_CRIT")))
df_fusion = spark.sql(sql_fusion_str)
# df_source.cache()
# df_fusion.cache()
