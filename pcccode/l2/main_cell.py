...
from pcccode.l2.src.running.cell import CellPcccode
from pcccode.l2.logic.stage_args import CellLogicArgs
...
if (JOB_RUN and IS_RUN == "TRUE"):
    ...
    try:
        ...
        Steps = {
            "pcccode": KpiInfo("None", ['L3C','L3D','L5B','L5C','L5D','L6A','L6B','L6K','L7A','L8A','L8B'], logic_args)
        }
...
from pcccode.l2.utils.utils import change_process_stage_name


def try_run_sql(sql: str) -> Optional[SparkDataFrame]:
    try:
...
sql_query = "SELECT * FROM auofab.itgp_eng.l3c_array_h_dax_fbk_pcccode_ods"
result_df = try_run_sql(sql_query)
